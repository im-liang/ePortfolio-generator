...
function initEPortfolio() {
    ...
    page_path = new Array();
    contentDataFile = "./data/content.json";
    // contentDataFile = new Array();
    image_caption = ["Saab", "Volvo", "BMW"];
...
function initPage() {
    ...
    $("#student_name").append("<p id = banner >" + banner +"</p>");
    $("#student_name").append("<img id = banner_img class = img src = '" + banner_path + banner_name + "'>");
        ...
        switch (components[i].component_type) {
            ...
            case 'slideshow':
                ...
                if(image_name.length > 0) {
                    ...
                    $("#components").append("<div id = caption>" + image_caption[currentSlide] + "</div>");
...
function fadeInCurrentSlide() {
    ...
    $(this).attr("src", filePath).bind('onreadystatechange load', function(){
        ...
        var cap = image_caption[currentSlide];
        $("#caption").html(cap);
        autoScaleImage();
        }
    });
